...
def cal_laplacian_loss(foreground_img, foreground_mask, blended_img, background_mask):
    ...
    lap_kernel = lap_kernel.view(1, 1, 3, 3).repeat(1, 3, 1, 1)
    lap_kernel = lap_kernel.to(foreground_img.device)
    
    lap_foreground_img = nn.functional.conv2d(foreground_img, lap_kernel, padding=1)
    lap_blended_img = nn.functional.conv2d(blended_img, lap_kernel, padding=1)
    
    foreground_mask_index = foreground_mask.bool()
    background_mask_index = background_mask.bool()
    
    loss = loss + torch.sum((lap_foreground_img[foreground_mask_index] - lap_blended_img[background_mask_index])**2)
